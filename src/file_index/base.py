...
class FileIndex:
    ...
    def hexdigest(self) -> str:
        """
        A forty character hash code of the paths in the index, obtained using the `sha1`
        algorithm.
        """
        hash_algorithm = sha1()
    ...
    def get_tag_groups(
        ...
        groups: set[frozenset[tuple[str, str]]] = {
            frozenset((k, v) for k, v in self.tags_by_paths[path].items() if k in keys)
            for path in paths
        }
    ...
    def remove(self, path: Path) -> None:
        for key, value in self.tags_by_paths[path].items():
            self.paths_by_tags[key][value].remove(path)
        del self.tags_by_paths[path]
